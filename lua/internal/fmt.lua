...
local function get_format_opt()
  ...
  if fmt_tools[vim.bo.filetype] then return fmt_tools[vim.bo.filetype] end
  return nil
end
...
function fmt:format_file(err, data, buf_data)
  ...
  if next(new_content) == nil then return end

  if not api.nvim_buf_is_valid(buf_data.buffer) then return end

  local curr_changedtick = api.nvim_buf_get_changedtick(buf_data.buffer)

  if buf_data.initial_changedtick ~= curr_changedtick then return end

  if string.len(new_content[#new_content]) == 0 then table.remove(new_content, #new_content) end
...
function fmt:new_spawn(buf_data)
  ...
  uv.read_start(stdout, vim.schedule_wrap(function(err, data) self:format_file(err, data, buf_data) end))

  uv.read_start(stderr, function(err, _) assert(not err, err) end)

  local ft = api.nvim_buf_get_option(buf_data.buffer, 'filetype')

  if ft == 'lua' or ft == 'markdown' then uv.write(stdin, buf_data.contents_with_wrap) end

  uv.shutdown(stdin, function() safe_close(stdin) end)
end
...
function fmt:formatter()
  ...
  if self[curr_buf] and self[curr_buf].initial_changedtick == curr_changedtick then return end
...
local use_format_tool = function(dir)
  if not format_tool_confs[vim.bo.filetype] then return false end

  if fn.filereadable(dir .. '/' .. format_tool_confs[vim.bo.filetype]) == 1 then return true end

  return false
end

function fmt:lsp(client)
