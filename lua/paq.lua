...
local function new_counter()
  return coroutine.wrap(function(op, total)
    ...
    while c.ok + c.err + c.nop < total do
      ...
      c[res] = c[res] + 1
      if res ~= 'nop' or cfg.verbose then report(over_op or op, name, res, c.ok + c.nop, total) end
    end
...
local function call_proc(process, args, cwd, cb, print_stdout)
  ...
  if not handle then vim.notify(string.format(' Paq: Failed to spawn %s (%s)', process, pid)) end
end
  ...
  for _, packdir in pairs({ 'start', 'opt' }) do
    ...
    while handle do
      ...
      if t == 'directory' and name ~= 'paq-nvim' then
        ...
        if not pkg or pkg.dir ~= dir then table.insert(unlisted, { name = name, dir = dir }) end
...
local function register(args)
  ...
  local name = args.as or url:gsub('%.git$', ''):match('/([%w-_.]+)$') -- Infer name from `url`
  if not name then return vim.notify(' Paq: Failed to parse ' .. vim.inspect(args), vim.log.levels.ERROR) end
...
return setmetatable({
  install = function()
    exe_op(
      'install',
      clone,
      vim.tbl_filter(function(pkg) return not pkg.exists and pkg.status ~= 'removed' end, packages)
    )
  end,
  update = function()
